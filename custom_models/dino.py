...
class FlaxDinov2Embeddings(nn.Module):
    ...
    def setup(self):
        ...
        self.patch_embeddings = FlaxDinov2PatchEmbeddings(
    ...
    def interpolate_pos_encoding(self, config, hidden_states, height, width, position_embeddings):
        ...
        scale = jnp.array(
            [new_height_ratio, new_width_ratio], dtype=jnp.float32)
    ...
    def __call__(self, inputs, deterministic: Optional[bool] = True):
        ...
        if deterministic:
            ...
            random_tensor = keep_prob + \
                jax.random.uniform(rng, shape=shape, dtype=inputs.dtype)
...
class FlaxDinov2PreTrainedModel(FlaxPreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if input_shape is None:
            input_shape = (1, config.image_size,
    ...
    def init_weights(self, rng: jax.random.PRNGKey, input_shape: Tuple, params: FrozenDict = None) -> FrozenDict:
        ...
        rngs = {"params": params_rng,
                "dropout": dropout_rng, "droppath": droppath_rng}

        random_params = self.module.init(
            rngs, pixel_values, return_dict=False)["params"]
...
class CheckpointableFlaxDinov2Model(FlaxDinov2PreTrainedModel):
...
class FlaxDinov2ForImageClassificationModule(nn.Module):
    ...
    def __call__(
        ...
        linear_input = jnp.concatenate(
            [cls_token, patch_tokens.mean(axis=1)], axis=-1)
...
overwrite_call_docstring(FlaxDinov2ForImageClassification,
                         FLAX_VISION_CLASSIFICATION_DOCSTRING)
append_replace_return_docstrings(
