...
def save_fused_image(fused_image: np.ndarray, output_dir: str = './out_images') -> None:
    """
...
def save_image(img: np.ndarray, step: int, window_num: int, out_dir: str = "./out_images") -> str:
    ...
    path = os.path.join(out_dir, f"RGB_obs_step{step}_{ts}_{window_num}.png")
    cv2.imwrite(path, img_bgr)
    return path


def crop_top_fraction(image: ImageRGB, top_frac: float = 1/3) -> ImageRGB:
    """Remove the top fraction (e.g., sky)."""
    cut = int(round(image.shape[0] * top_frac))
    return image[cut:, :, :]
...
class RoverEnvActivVis(gym.Env):
    ...
    def __init__(self, size=(96, 96), length=4000, scan_topic='/scan', imu_topic='/imu/data',
        ...
        self.out_hw_lut = 640   # build remap at 144x144 (AA render size)
        self.out_hw = 320 #96
        self.top_frac = 1/3
        ...
        # Attach to camera_latest SHM (for writing TO inference)
    ...
    def get_camera_window(self) -> ImageRGB:
        """Return a 96x96 rectified 60° pinhole view at the current yaw bin."""
        # Wait for an image (same as before)
        timeout = 2.0
        t0 = time.time()
        while self.latest_fisheye_image is None:
            rclpy.spin_once(self.node, timeout_sec=0.01)
            if time.time() - t0 > timeout:
                raise TimeoutError("No camera image received")

        # Crop top fraction (e.g., 1/3) -> 1600x400
        full_img = self.latest_fisheye_image
        cropped = crop_top_fraction(full_img, top_frac=self.top_frac)

        # Build LUT bank once (depends on src size and HFOV only)
        if self._lut_bank is None:
            H_full = full_img.shape[0]
            crop_top_px = int(round(H_full * self.top_frac))
            Hc, Wc = cropped.shape[0], cropped.shape[1]

            self._lut_bank = build_lut_bank(
                src_size=(Wc, Hc),
                hfov_src=self.hfov_src,
                yaws_deg=self.window_yaws_deg,
                hfov_win_deg=60.0,
                out_hw=self.out_hw_lut,   # was self.out_hw
                crop_top_px=crop_top_px
            )

        idx = int(np.clip(self.current_window_index, 0, len(self.window_yaws_deg)-1))

        # 1) remap to 144x144 (using the precomputed LUT)
        hi = extract_view_with_lut(cropped, self._lut_bank[idx])        # -> (144,144,3)

        # 2) downsample to 96x96 with area filter (antialiasing)
        lo = cv2.resize(hi, (self.out_hw, self.out_hw), interpolation=cv2.INTER_AREA)
        
        return lo

    def write_frame_to_inference(self, frame_320: np.ndarray) -> float:
        """Write 320x320 frame to inference SHM"""
        timestamp = self.node.get_clock().now().nanoseconds / 1e9  # Gazebo time
        struct.pack_into('<d', self.camera_shm.buf, 0, timestamp)
        frame_bytes = frame_320.astype(np.uint8).tobytes()
        self.camera_shm.buf[8:8+len(frame_bytes)] = frame_bytes
        return timestamp
    ...
    def get_fused_observation(self) -> np.ndarray:
        ...
        while True:
            ...
            if not hasattr(self, "_last_obs_ts"):
                self._last_obs_ts = -1.0  # Start with impossible value

            # On first call, accept any valid observation
            # On subsequent calls, wait for NEW timestamp
            first_call = (self._last_obs_ts == -1.0)

            if valid_flag == 1 and (first_call or ts != self._last_obs_ts):
    ...
    def task_reward(self, observation):
        ...
        if self.total_steps % 2_000 == 0:
            if self.total_steps % 4_000 == 0:
                save_fused_image(observation['image'], self.total_steps, self.current_window_index)
            # print (as you had)
    ...
    def step(self, action):
        ...
        # NEW: Get 320x320 frame and send to inference
        frame_320 = self.get_camera_window()  # Now returns 320x320
        self.write_frame_to_inference(frame_320)

        if self.total_steps % 4_000 == 0:
            save_image(frame_320, self.total_steps, self.current_window_index)
        ...
        if self._step >= self._length:
            ...
            return observation, 0.0, True, {'steps': self._step, 'total_steps': self.total_steps,
                                                       'reward': 0.0}
    ...
    def reset(self, seed=None, options=None):
        ...
        # Send initial frame to inference
        frame_320 = self.get_camera_window()
        self.write_frame_to_inference(frame_320)
        
        # Spin ROS to let camera frames accumulate (inference needs 5+ frames)
        for _ in range(100):  # ~1 second for frames to arrive
            rclpy.spin_once(self.node, timeout_sec=0.01)
        
        # Now get the fused observation
        fused_obs = self.get_observation()
