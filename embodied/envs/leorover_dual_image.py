...
class RoverEnvActivVis(gym.Env):
    ...
    def __init__(self, size=(96, 96), length=4000, scan_topic='/scan', imu_topic='/imu/data',
        ...
        self.max_lidar_range = msg.range_max if hasattr(msg, "range_max") else 10.0
        self.lidar_data = np.full(self.lidar_points, self.max_lidar_range, dtype=np.float32)
    ...
    def lidar_callback(self, msg):
        lidar = np.asarray(msg.ranges, dtype=np.float32)

        # Sanitize
        max_r = msg.range_max
        lidar[np.isnan(lidar) | np.isinf(lidar)] = max_r
        lidar = np.clip(lidar, 0.0, max_r)
        
        # Original scan angles
        angles = msg.angle_min + np.arange(len(lidar)) * msg.angle_increment
        
        # Target fixed bins
        target_angles = np.linspace(-np.pi, np.pi, self.lidar_points, endpoint=False)
        
        out = np.full(self.lidar_points, max_r, dtype=np.float32)
        
        # Assign each raw ray to nearest bin
        idx = np.argmin(np.abs(angles[:, None] - target_angles[None, :]), axis=1)
