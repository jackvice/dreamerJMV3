...
class RoverEnvFused(gym.Env):
    ...
    def __init__(self, size=(96, 96), length=6000, scan_topic='/scan', imu_topic='/imu/data',
        ...
        self.observation_space = spaces.Dict({
            ...
            "imu":         spaces.Box(low=-np.pi, high=np.pi, shape=(3,), dtype=np.float32),
            "pose":        spaces.Box(low=np.array([-30, -30, -np.pi], np.float32),
                                      high=np.array([10, 10, np.pi], np.float32), dtype=np.float32),
            "target":      spaces.Box(low=-np.pi, high=np.pi, shape=(2,), dtype=np.float32),
            "velocities":  spaces.Box(low=-10.0, high=10.0, shape=(2,), dtype=np.float32),
        })
