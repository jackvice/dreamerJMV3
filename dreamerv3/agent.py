...
f32 = jnp.float32
i32 = jnp.int32
def sg(xs, skip=False): return xs if skip else jax.lax.stop_gradient(xs)
def sample(xs): return jax.tree.map(lambda x: x.sample(nj.seed()), xs)
def prefix(xs, p): return {f'{p}/{k}': v for k, v in xs.items()}
def concat(xs, a): return jax.tree.map(lambda *x: jnp.concatenate(x, a), *xs)
def isimage(s): return s.dtype == np.uint8 and len(s.shape) == 3
...
class Agent(embodied.jax.Agent):
    ...
    def init_policy(self, batch_size):
        def zeros(x): return jnp.zeros((batch_size, *x.shape), x.dtype)
        return (
    ...
    def loss(self, carry, obs, prevact, training):
        ...
        def policyfn(feat): return sample(self.pol(self.feat2tensor(feat), 1))
        _, imgfeat, imgprevact = self.dyn.imagine(
            starts, policyfn, H, training)

        # concatenate the starting state to the imagined sequence of states
        first = jax.tree.map(
            lambda x: x[:, -K:].reshape((B * K, 1, *x.shape[2:])), repfeat)
        imgfeat = concat(
            [sg(first, skip=self.config.ac_grads), sg(imgfeat)], 1)

        # we sample a final action such that we have the same number of imagined states as actions
        lastact = policyfn(jax.tree.map(lambda x: x[:, -1], imgfeat))
        lastact = jax.tree.map(lambda x: x[:, None], lastact)
        imgact = concat([imgprevact, lastact], 1)
        assert all(x.shape[:2] == (B * K, H + 1)
                   for x in jax.tree.leaves(imgfeat))
        assert all(x.shape[:2] == (B * K, H + 1)
                   for x in jax.tree.leaves(imgact))
        ...
        if self.config.repval_loss:
            ...
            last, term, rew = [obs[k]
                               for k in ('is_last', 'is_terminal', 'reward')]
    ...
    def report(self, carry, data):
        ...
        if self.config.report_gradnorms:
            for key in self.scales:
                try:
                    def lossfn(data, carry): return self.loss(
        ...
        # Open loop
        def firsthalf(xs): return jax.tree.map(lambda x: x[:RB, :T // 2], xs)
        def secondhalf(xs): return jax.tree.map(lambda x: x[:RB, T // 2:], xs)
        ...
        for key in self.dec.imgkeys:
            ...
            pred = jnp.concatenate(
                [obsrecons[key].pred(), imgrecons[key].pred()], 1)
            ...
            mask = jnp.zeros(
                video.shape, bool).at[:, :, 2:-2, 2:-2, :].set(True)
    ...
    def _apply_replay_context(self, carry, data):
        ...
        def prepend(x, y): return jnp.concatenate([x[:, None], y[:, :-1]], 1)
        ...
        def lhs(xs): return jax.tree.map(lambda x: x[:, :K], xs)
        def rhs(xs): return jax.tree.map(lambda x: x[:, K:], xs)
        rep_carry = (
    ...
    def _make_opt(
        ...
        def make_chain(learning_rate):
            ...
            if wd:
                ...
                def wdmask(params): return {k: bool(
                    pattern.search(k)) for k in params}
            ...
            if schedule == 'const':
                ...
                sched = optax.linear_schedule(
                    learning_rate, 0.1 * learning_rate, anneal - warmup)
            elif schedule == 'cosine':
                sched = optax.cosine_decay_schedule(
                    learning_rate, anneal - warmup, 0.1 * learning_rate)
            else:
